...
namespace sjtu
{
    ...
    public:
        ...
        OperationResult erase(const Key& key)
        {
            ...
            if (i == leaf.num)return Fail;
            for (int j = i; j < leaf.num-1; ++j)
            {
                leaf.data[j].first = leaf.data[j + 1].first;
                leaf.data[j].second = leaf.data[j + 1].second;
            }
            --leaf.num;
            int poffset = leaf.parent;
            internalnode node;
            while (i == 0)
            {
                if (poffset == 0)break;
                read(&node, poffset, 1, sizeof(internalnode));
                i = 0;
                for (; i < node.num; ++i)
                {
                    if (node.key[i] == key)
            ...
            write(&leaf, index, 1, sizeof(leafnode));
            return Success;
        }
